...
async function fetchRawRepoHealth(
  ...
  while (hasNextIssues || hasNextPrs) {
    const response: GitHubGraphqlResponse = await githubGraphql<GitHubGraphqlResponse>(
      token,
      REPO_HEALTH_QUERY,
      {
        owner,
    ...
    issues.push(
      ...repository.issues.nodes.map((issue: GraphqlIssueNode) => ({
        ...
        comments: issue.comments.nodes.map((comment: { createdAt: string; author: { login: string | null; __typename: string | null } | null }) => ({
    ...
    pullRequests.push(
      ...repository.pullRequests.nodes.map((pr: GraphqlPullRequestNode) => ({
        ...
        comments: pr.comments.nodes.map((comment: { createdAt: string; author: { login: string | null; __typename: string | null } | null }) => ({
          createdAt: comment.createdAt,
          author: comment.author,
        })),
        reviews: pr.reviews.nodes.map((review: { createdAt: string; author: { login: string | null; __typename: string | null } | null }) => ({
